...
#### 4장 동물입니까, 식물입니까, 무생물입니까?
  ...
  - 방법
    - 간결성 : 핵심만 남기라는 뜻
      - 환영 문구, 장황한 소개, 불필요한 지시사항은 다 지워라.
      - 긴 문장은 짧은 문장으로, 긴 단락은 여러 개의 짧은 단락으로 나눠라.
      - 핵심만 남기고 나머지는 버려라. 사용자는 소설을 읽으러 온 게 아니다.
    - 적시성 : 시의 적절하게 보여주라는 뜻
      - 홈페이지에서부터 모든 도움말을 보여주지 마라. 사용자가 막혔을 때, 그 맥락에 맞는 도움말 링크를 바로 옆에 제공해라.
      - 결제 과정에서 배송지 정보를 입력할 때, 배송 옵션과 비용을 보여줘라. 상품 페이지에서부터 보여줄 필요는 없다.
      - 양식을 다 채우기 전에 오류 메시지를 띄우지 말고, '제출' 버튼을 눌렀을 때 빠진 부분을 알려줘라.
    - 불가피성 : 절대 놓칠 수 없게 하라는 뜻
      - 페이지의 가장 중요한 기능(예: '장바구니에 담기', '결제하기')은 시각적으로 가장 두드러지게 만들어라. 크기, 색상, 대비, 위치를 활용해라.
      - 사이트 전체에 적용되는 공지사항(예: 배송 지연 안내)은 모든 페이지 상단에 눈에 띄는 배너 형태로 배치해라.
      - 시각적 계층 구조를 명확히 해서, 사용자의 시선이 자연스럽게 가장 중요한 요소부터 덜 중요한 요소 순으로 흐르게 설계해라.
